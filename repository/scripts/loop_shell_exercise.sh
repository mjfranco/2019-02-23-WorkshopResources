...
do
    ...
    then  # if "high" then show highest value (tail)
            cut -f "$1" $gapminderfile | sort -n | tail -1 >> gm_field"$1".out #Capture output into file
    else  # otherwise show lowest value (head)
            cut -f "$1" $gapminderfile | sort -n | head -1 >> gm_field"$1".out #Capture output into file
    fi
done
